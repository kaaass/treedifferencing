...
/**
 * An implementation of the so-called Hungarian Algorithm (Kuhn-Munkres
 * algorithm) for the linear assignment problem. It is based on
...
public final class Hungarian {
	...
	private static void addSmallest(final DoubleMatrix matrix, final ByteMatrix mask, final boolean[] rowCover,
	...
	private static void alternatingSeries(final DoubleMatrix matrix, final ByteMatrix mask, final boolean[] rowCover,
			final boolean[] colCover, final int pathRow0, final int pathCol0) {
		// Step 5
		...
		boolean done = false;
		do {
			int j = path.get(pathCount - 1).col;
			int i = findStarInCol(mask, j);
			if (i > -1) {
				pathCount += 1;
			...
			if (!done) {
				i = path.get(pathCount - 1).row;
				j = findPrimeInRow(mask, i);
				pathCount += 1;
				path.add(new Pair(i, j));
			}
		} while (!done);
	...
	/**
	 * Find a minimizing solution for the linear assignment problem specified by
	 * the matrix.
	 *
	 * @return An array with an element for each row containing the assigned
	...
	private static void augmentPath(final ByteMatrix mask, final int pathCount, final ArrayList<Pair> path) {
	...
	private static void coverColumns(final DoubleMatrix matrix, final ByteMatrix mask, final boolean[] rowCover,
			final boolean[] colCover) {
		// Step 3
		final int n = matrix.numRows();
	...
	// == Helpers ==
	private static Pair findAZero(final DoubleMatrix matrix, final ByteMatrix mask, final boolean[] rowCover,
			final boolean[] colCover) {
		final int n = matrix.numRows();
	...
	private static void findNonCovered(final DoubleMatrix matrix, final ByteMatrix mask, final boolean[] rowCover,
			final boolean[] colCover) {
		// Step 4
		int row = -1;
		int col = -1;
	...
	private static double findSmallest(final DoubleMatrix matrix, final boolean[] rowCover, final boolean[] colCover) {
		final int n = matrix.numRows();
	...
	private static int findXInRow(final ByteMatrix mask, final int row, final int x) {
		final int n = mask.numRows();
