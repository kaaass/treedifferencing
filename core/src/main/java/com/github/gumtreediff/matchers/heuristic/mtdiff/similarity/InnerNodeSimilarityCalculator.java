...
public class InnerNodeSimilarityCalculator {
  ...
  boolean match(float labelSimilarity, float combinedSimilarity, ITree first, ITree second) {
  ...
  boolean match(ITree first, ITree second, float similarity) {
  ...
  float newSimilarity(ITree first, ITree second, float labelSim) {
    float childSim = 0.0f;
    if (first.getChildren() == null || first.getChildren().size() == 0) {
      childSim = 0.0f;
  ...
  float mtDiffSimilarity(ITree first, ITree second, float labelSim) {
    float childSim = 0.0f;
    if (first.getChildren() == null || first.getChildren().size() == 0) {
      childSim = 0.0f;
    } else if (first.getType() != second.getType()) {
  ...
  float similarity(ITree first, ITree second) {
    if (first.getType() != second.getType()) {
      return 0.0f;
    }
