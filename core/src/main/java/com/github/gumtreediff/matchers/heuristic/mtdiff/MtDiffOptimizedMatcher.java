...
package extern.com.github.gumtreediff.matchers.heuristic.mtdiff;
...
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.InnerMatcherMtDiffRunnable;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.InnerNodeMatcher;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.LMatcher;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.LabelConfiguration;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.LeafSimResults;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.LeafSimilarityRunnable;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.MatchingCandidate;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.OptimizedTreeDifferenceRunnable;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.PairComparator;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.PostOrderSetGenerator;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.TagComparator;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.intern.TreeMatcherConfiguration;
...
public class MtDiffOptimizedMatcher extends Matcher {
  ...
  private void computeMatchingPairs() throws Exception {
    ...
    final ConcurrentHashMap<String, Float> stringSimCache = new ConcurrentHashMap<>(HASH_MAP_SIZE);
    ...
    matchLeaves(orderedList1, orderedList2, matchedLeaves, resultMap, unmatchedNodes1,
        unmatchedNodes2, onlyOneClassPair, list, stringSimCache, leaves1tmp, leaves2tmp, skipList,
        lMatcher, parents1, parents2, leavesMap1, leavesMap2, directChildrenMap1,
    ...
    matchInnerNodes(orderedList1, orderedList2, resultMap, unmatchedNodesOrdered1,
        unmatchedNodesOrdered2, leavesMap1, directChildrenMap1, leavesMap2, directChildrenMap2,
        skipList, resultSet);
  ...
  private void matchInnerNodes(IdentityHashMap<ITree, Integer> orderedList1,
    ...
    InnerNodeMatcher innerMatcher2 = new InnerNodeMatcher(labelConfiguration, leavesMap1,
        leavesMap2, directChildrenMap1, directChildrenMap2, resultSet);
    ...
    while (start + step < unmatchedNodesOrdered1.size()) {
      InnerMatcherMtDiffRunnable runnable = new InnerMatcherMtDiffRunnable(unmatchedNodesOrdered1,
          unmatchedNodesOrdered2, innerMatchCounter, start, start + step, innerMatcher2);
    ...
    InnerMatcherMtDiffRunnable runnable =
        new InnerMatcherMtDiffRunnable(unmatchedNodesOrdered1,
            unmatchedNodesOrdered2, innerMatchCounter,
            start, unmatchedNodesOrdered1.size(), innerMatcher2);
    if (executorService != null) {
      results.add(executorService.submit(runnable));
    } else {
      FutureTask<ArrayList<MatchingCandidate>> task = new FutureTask<>(runnable);
  ...
  private void matchLeaves(IdentityHashMap<ITree, Integer> orderedList1,
      ...
      Set<ITree> unmatchedNodes2, boolean onlyOneClassPair, List<Map.Entry<Integer, Integer>> list,
      ConcurrentHashMap<String, Float> stringSimCache, ArrayList<ITree> leaves1tmp,
      ArrayList<ITree> leaves2tmp, HashSet<ITree> skipList, LMatcher leafMatcher,
      Map<ITree, ITree> parents1, Map<ITree, ITree> parents2,
    ...
    for (Map.Entry<Integer, Integer> tagEntry : list) {
      ...
      while (start + step < nodes.length) {
        LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes,
            subLeaves2, leafSimCounter, start, start + step, orderedList1, orderedList2, skipList,
            leafMatcher, labelConfiguration, renames);
        if (executorService != null) {
      ...
      LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes,
          subLeaves2, leafSimCounter, start, nodes.length, orderedList1, orderedList2, skipList,
          leafMatcher, labelConfiguration, renames);
      if (executorService != null) {
        leafSimResults.add(executorService.submit(runnable));
      } else {
      ...
      for (Future<LeafSimResults<ITree>> fu : leafSimResults) {
        ...
        try {
          ...
          for (final Map.Entry<ITree, ArrayList<MatchingCandidate>> entry 
              : leafSimRes.subleafCandidateMap
              .entrySet()) {
            ArrayList<MatchingCandidate> myList = leafCandidateMap.putIfAbsent(
      ...
      Map.Entry<ITree, ArrayList<MatchingCandidate>>[] array =
      ...
      for (int i = 0; i < oldNodeArray.size(); i++) {
        ...
        OptimizedTreeDifferenceRunnable treeDiffRunnable =
            new OptimizedTreeDifferenceRunnable(oldNodeList, newNodeList, subList, treeDiffCounter,
                stringSimCache, onlyOneClassPair, orderedList1, orderedList2, resultMap,
                similarityCache, similarityEntries, parents1, parents2, leavesMap1, leavesMap2,
                labelConfiguration, leafMatcher, directChildrenMap1, directChildrenMap2, getSrc(),
