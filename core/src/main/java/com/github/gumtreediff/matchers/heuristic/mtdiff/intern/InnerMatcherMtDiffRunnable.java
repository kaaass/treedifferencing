...
import common.settings.EmgIterationSettings;
...
public class InnerMatcherMtDiffRunnable implements Callable<ArrayList<MatchingCandidate>> {
  ...
  public InnerMatcherMtDiffRunnable(ArrayList<ITree> unmatchedNodesOrdered1,
    ...
    this.weightValue = EmgIterationSettings.getInstance().getWeightInnerValueSim();
    this.weightChildren = EmgIterationSettings.getInstance().getWeightInnerChildrenSim();
    this.innerNodeThreshold = EmgIterationSettings.getInstance().getInnerNodeThreshold();
