...
import com.github.gumtreediff.matchers.heuristic.cdopt.hungarian.DoubleMatrix;
import com.github.gumtreediff.matchers.heuristic.cdopt.hungarian.Hungarian;
import com.github.gumtreediff.matchers.heuristic.cdopt.similarity.NGramCalculator;
import com.github.gumtreediff.tree.ITree;

public class RestructuredTreeDiffHelperRunnable implements Callable<Boolean> {

	public static final int NODE_AGGREGATION_LABEL = -1000;

	public static double[][] performTransformation(double[][] original) {
	...
	Map<ITree, ITree> parents1, parents2;
	...
	public RestructuredTreeDiffHelperRunnable(boolean[][] aggregationFinished, ITree[] firstAggregations,
			ITree[] secondAggregations, ConcurrentHashMap<ITree, MatchingCandidate> currentResultMap,
			AtomicBoolean changed, ArrayList<ITree> oldNodes, ArrayList<ITree> newNodes,
			IdentityHashMap<ITree, Mapping> resultMap, NGramCalculator stringSim,
			ConcurrentHashMap<String, Float> stringSimCache, boolean onlyOneClassPair, double[][] similarityScores,
			ConcurrentSkipListSet<MatchingCandidate> initialList,
			ConcurrentHashMap<ITree, ConcurrentHashMap<ITree, MatchingCandidate>> candidateMap,
			AtomicIntegerArray foundMaxArray, ConcurrentHashMap<ITree, ConcurrentHashMap<ITree, Float>> similarityCache,
			...
			LabelConfiguration labelConfiguration, LMatcher lMatcher, Map<ITree, ArrayList<ITree>> directChildrenMap1,
			Map<ITree, ArrayList<ITree>> directChildrenMap2, ITree root1, ITree root2, double weightSimilarity,
		...
		this.futures = new Future[oldNodes.size()];
	...
	public Boolean call() {
		try {
			while (changed.get()) {
				...
				for (int i = 0; i < oldNodes.size(); i++) {
					updateSimilarityRow(aggregationFinished, similarityScores, firstAggregations, secondAggregations,
							currentResultMap, changed, i, newNodes, onlyOneClassPair, resultMap, stringSim,
							stringSimCache, similarityCache, similarityEntries);
				}
			}
			...
			for (int i = 0; i < oldNodes.size(); i++) {
				for (int j = 0; j < newNodes.size(); j++) {
					if (root1.getId() > oldNodes.get(i).getId()) {
						...
						int val = Math.abs(
								(oldNodes.get(i).getId() - root1.getId()) - (newNodes.get(j).getId() - root2.getId()));
						posDiff[i][j] = val;
						if (val > maxDiff) {
							maxDiff = val;
						}
	...
	/**
	 * Creates a {@link diff.tree.matching.NodeAggregation} that consists of the
	 * specified node, it's parent and all of it's siblings.
	 */
	...
	private float simpleSimilarity(final ITree tree1, final ITree tree2,
		...
		if (tree1.getType() == labelConfiguration.rootLabel && tree2.getType() == labelConfiguration.rootLabel) {
			return 1.0f;
		} else if (tree1.getType() == labelConfiguration.rootLabel || tree2.getType() == labelConfiguration.rootLabel) {
			return 0.0f;
		}
		...
		if (tree1.getType() == labelConfiguration.classLabel && tree2.getType() == labelConfiguration.classLabel) {
			if (onlyOneClassPair) {
				return 1.0f;
			}
		...
		for (int tag : nodeTags) {
			...
			for (final ITree firstNode : subLeaves1) {
				...
				for (final ITree secondNode : subLeaves2) {
					...
					} else if (tag == labelConfiguration.basicTypeLabel || tag == labelConfiguration.qualifierLabel) {
		...
		IdentityHashMap<ITree, Integer> orderedList1 = CdOptimizedMatcher.getNodesInOrder(tree1);
		IdentityHashMap<ITree, Integer> orderedList2 = CdOptimizedMatcher.getNodesInOrder(tree2);
		...
		InnerNodeMatcher nodeMatcher = new InnerNodeMatcher(labelConfiguration, leavesMap1, leavesMap2,
	...
	private void updateSimilarityRow(boolean[][] aggregationFinished, double[][] similarityScores,
			...
			ConcurrentHashMap<ITree, MatchingCandidate> currentResultMap, AtomicBoolean changed, int i,
			ArrayList<ITree> newNodes, boolean onlyOneClassPair, IdentityHashMap<ITree, Mapping> resultMap,
			NGramCalculator stringSim, ConcurrentHashMap<String, Float> stringSimCache,
			ConcurrentHashMap<ITree, ConcurrentHashMap<ITree, Float>> similarityCache, AtomicLong similarityEntries) {
		{

			for (int j = 0; j < newNodes.size(); j++) {
				if (!aggregationFinished[i][j]) {
					if (firstAggregations[i] != null && firstAggregations[i].getType() != NodeAggregation.tag) {
						firstAggregations[i] = new NodeAggregation(firstAggregations[i]);
					}
					if (secondAggregations[j] != null && secondAggregations[j].getType() != NodeAggregation.tag) {
						secondAggregations[j] = new NodeAggregation(secondAggregations[j]);
					}
					final NodeAggregation firstAggregation = (NodeAggregation) firstAggregations[i];
					...
					if (firstAggregation == null || firstAggregation.getAssociatedTree() == null
						...
						ConcurrentHashMap<ITree, Float> map = similarityCache.get(firstAggregation.getAssociatedTree());
							...
							if (leavesMap1.get(firstAggregation.getAssociatedTree()).size()
								...
								if (map != null) {
									...
									if (value != null) {
										...
										if (similarityEntries.get() < CdOptimizedMatcher.SIMILIARITY_CACHE_SIZE) {
