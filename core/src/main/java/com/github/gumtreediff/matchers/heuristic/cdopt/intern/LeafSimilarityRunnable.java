...
public class LeafSimilarityRunnable implements Callable<LeafSimResults<ITree>> {
	...
	public LeafSimResults<ITree> call() throws Exception {
		try {
			ConcurrentHashMap<ITree, ArrayList<MatchingCandidate>> leafCandidateMap = new ConcurrentHashMap<>();
			ConcurrentSkipListSet<MatchingCandidate> matchedLeaves = new ConcurrentSkipListSet<>(
			...
			for (int i = start; i < end; i++) {
					...
					if (basicTypeCache.containsKey(firstNode.getLabel())) {
						...
						if (oldList != null) {
							for (MatchingCandidate mc : oldList) {
								MatchingCandidate newMC = new MatchingCandidate(firstNode, mc.second, mc.getValue());
								newList.add(newMC);
				...
				computeLeafSimilarities(matchedLeaves, stringSimCache, leafCandidateMap, subLeaves2, stringSim,
						firstNode, skipList, lmatcher);
				if (firstNode.getType() == labelConfiguration.basicTypeLabel) {
					if (!basicTypeCache.containsKey(firstNode.getLabel())) {
						basicTypeCache.put(firstNode.getLabel(), firstNode);
					}
				} else if (firstNode.getType() == labelConfiguration.identifierLabel) {
					if (!basicNameCache.containsKey(firstNode.getLabel())) {
						basicNameCache.put(firstNode.getLabel(), firstNode);
					}
				} else if (firstNode.getType() == labelConfiguration.qualifierLabel) {
					if (!basicTypeQualifierCache.containsKey(firstNode.getLabel())) {
						basicTypeQualifierCache.put(firstNode.getLabel(), firstNode);
					}
				}
			}
	...
	private void computeLeafSimilarities(final ConcurrentSkipListSet<MatchingCandidate> matchedLeaves,
			...
			final ConcurrentHashMap<ITree, ArrayList<MatchingCandidate>> leafCandidateMap, ArrayList<ITree> subLeaves2,
			NGramCalculator stringSim, final ITree firstNode, HashSet<ITree> skipList, LMatcher lmatcher) {
		...
		for (final ITree secondNode : subLeaves2) {
			...
			if (renames != null && labelConfiguration.labelsForStringCompare.contains(firstNode.getType())) {
	...
	/**
	 * Inserts a {@link MatchingCandidate} into a specified mapping.
	 * 
	 * @param candidateMapping
	 *            The mapping of a node to all possible candidates.
	 * @param key
	 *            The ITree to be inserted as a key
	 * @param candidate
	 *            The candidate to be inserted as a value
	 */
	private void insertCandidate(final Map<ITree, ArrayList<MatchingCandidate>> candidateMapping, final ITree key,
			final MatchingCandidate candidate, ConcurrentSkipListSet<MatchingCandidate> matchedLeaves) {
