 ...
 */
package com.github.gumtreediff.matchers.heuristic.cdopt;
...
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.InnerMatcherRunnable;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.InnerNodeMatcher;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.LMatcher;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.LabelConfiguration;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.LeafSimResults;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.LeafSimilarityRunnable;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.MatchingCandidate;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.OptimizedTreeDifferenceRunnable;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.PairComparator;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.PostOrderSetGenerator;
import com.github.gumtreediff.matchers.heuristic.cdopt.intern.TagComparator;
...
/**
 * 
 * This implements the optimized change distiller that includes the
 * optimizations Phi A-E.
 *
 */
public class CdOptimizedMatcher extends Matcher {
  public static final double WEIGHT_POSITION = 0.003414f;

  public static final double WEIGHT_SIMILARITY = 0.408218f;
  public static final double LEAF_THRESHOLD = 0.776272f;
	...
	private static int numberOfNodesWithTag(ITree tree, int tag) {
		ConcurrentLinkedQueue<ITree> workList = new ConcurrentLinkedQueue<ITree>();
		workList.add(tree);
		int i = 0;
		while (!workList.isEmpty()) {
	...
	public CdOptimizedMatcher(ITree src, ITree dst, MappingStore store) {
		super(src, dst, store);
	}

	public void computeMatchingPairs() throws Exception {
		LMatcher lMatcher = new LMatcher(labelConfiguration, configuration.leafThreshold);
		...
		final ConcurrentSkipListSet<MatchingCandidate> matchedLeaves = new ConcurrentSkipListSet<>(
		...
		Set<ITree> leaves2 = null;
		Set<ITree> unmatchedNodes2 = null;
		...
		IdentityHashMap<ITree, ArrayList<ITree>> leavesMap1 = setGeneratorNew.getLeaveMap();
		IdentityHashMap<ITree, ArrayList<ITree>> directChildrenMap1 = setGeneratorNew.getDirectChildrenMap();
		setGeneratorNew.createSetsForNode(getDst());
		leaves2 = setGeneratorNew.getSetOfLeaves();
		unmatchedNodes2 = setGeneratorNew.getSetOfNodes();

		IdentityHashMap<ITree, ArrayList<ITree>> leavesMap2 = setGeneratorNew.getLeaveMap();
		IdentityHashMap<ITree, ArrayList<ITree>> directChildrenMap2 = setGeneratorNew.getDirectChildrenMap();
		...
		matchLeaves(orderedList1, orderedList2, matchedLeaves, resultMap, unmatchedNodes1, unmatchedNodes2,
				onlyOneClassPair, list, stringSimCache, leaves1tmp, leaves2tmp, skipList, lMatcher, parents1, parents2,
				leavesMap1, leavesMap2, directChildrenMap1, directChildrenMap2, null);
		...
		matchInnerNodes(orderedList1, orderedList2, resultMap, unmatchedNodesOrdered1, unmatchedNodesOrdered2,
				leavesMap1, directChildrenMap1, leavesMap2, directChildrenMap2, skipList, resultSet);

		resultSet = new HashSet<>();
		resultSet.addAll(resultMap.values());
		resultMap.clear();
		for (Mapping map : resultSet) {
			addMapping(map.first, map.second);
		}
	}

	private float computeNodesimilarity(ConcurrentHashMap<String, Float> stringSimCache, NGramCalculator stringSim,
			ITree firstNode, ITree secondNode, LMatcher lMatcher) {
		float sim = 0.0f;
	...
	public String getHash(ITree node, IdentityHashMap<ITree, Integer> quickFind,
			IdentityHashMap<ITree, String> stringMap) {
	...
	public List<ITree> getNodeStream(ITree root) {
	...
	public void initSIFE(ExecutorService executorService, TreeMatcherConfiguration configuration,
	...
	public void matchInnerNodes(IdentityHashMap<ITree, Integer> orderedList1,
		...
		InnerNodeMatcher innerMatcher2 = new InnerNodeMatcher(labelConfiguration, leavesMap1, leavesMap2,
		...
		while (start + step < unmatchedNodesOrdered1.size()) {
			InnerMatcherRunnable runnable = new InnerMatcherRunnable(unmatchedNodesOrdered1, unmatchedNodesOrdered2,
		...
		while ((unmatchedNodesOrdered1 != null && unmatchedNodesOrdered2 != null) && candidateList.size() > 0) {
			MatchingCandidate can = candidateList.pollLast();

			if (unmatchedNodesOrdered1.contains(can.first) && unmatchedNodesOrdered2.contains(can.second)) {
	...
	public void matchLeaves(IdentityHashMap<ITree, Integer> orderedList1, IdentityHashMap<ITree, Integer> orderedList2,
			final ConcurrentSkipListSet<MatchingCandidate> matchedLeaves, IdentityHashMap<ITree, Mapping> resultMap,
			Set<ITree> unmatchedNodes1, Set<ITree> unmatchedNodes2, boolean onlyOneClassPair,
			List<Map.Entry<Integer, Integer>> list, ConcurrentHashMap<String, Float> stringSimCache,
			ArrayList<ITree> leaves1tmp, ArrayList<ITree> leaves2tmp, HashSet<ITree> skipList, LMatcher lMatcher,
		...
		ConcurrentHashMap<ITree, ConcurrentHashMap<ITree, Float>> similarityCache = new ConcurrentHashMap<>(
				SIMILIARITY_CACHE_SIZE / 1000);
		for (Map.Entry<Integer, Integer> tagEntry : list) {
			int tag = tagEntry.getKey();
			final ConcurrentHashMap<ITree, ArrayList<MatchingCandidate>> leafCandidateMap = new ConcurrentHashMap<>();
			...
			while (start + step < nodes.length) {
				LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes, subLeaves2,
						leafSimCounter, start, start + step, orderedList1, orderedList2, skipList, lMatcher,
						labelConfiguration, renames);
				if (executorService != null) {
			...
			LeafSimilarityRunnable runnable = new LeafSimilarityRunnable(stringSimCache, nodes, subLeaves2,
					leafSimCounter, start, nodes.length, orderedList1, orderedList2, skipList, lMatcher,
					labelConfiguration, renames);
			if (executorService != null) {
				leafSimResults.add(executorService.submit(runnable));
			} else {
			...
			for (Future<LeafSimResults<ITree>> fu : leafSimResults) {
				...
				try {
					...
					for (final Map.Entry<ITree, ArrayList<MatchingCandidate>> entry : leafSimRes.subleafCandidateMap
							.entrySet()) {
						if (!leafCandidateMap.containsKey(entry.getKey())) {
							ArrayList<MatchingCandidate> myList = new ArrayList<>(entry.getValue());
							leafCandidateMap.put(entry.getKey(), myList);
						} else {
			...
			for (final Map.Entry<ITree, ArrayList<MatchingCandidate>> entry : leafCandidateMap.entrySet()) {
			...
			Map.Entry<ITree, ArrayList<MatchingCandidate>>[] array = (Entry<ITree, ArrayList<MatchingCandidate>>[]) tieCandidates
			.entrySet().toArray(new Map.Entry[tieCandidates.size()]);
			...
			for (Entry<ITree, ArrayList<MatchingCandidate>> tieEntries : array) {
				for (MatchingCandidate mc : tieEntries.getValue()) {
					if (!nodeMappings.containsKey(mc.first) && !(nodeMappings.containsKey(mc.second))) {
						...
						canList.add(mc);
					} else if (nodeMappings.containsKey(mc.first) && !nodeMappings.containsKey(mc.second)) {
			...
			for (Map.Entry<ITree, LinkedHashSet<ITree>> entry : nodeMappings.entrySet()) {
			...
			for (int i = 0; i < oldNodeArray.size(); i++) {
				...
				OptimizedTreeDifferenceRunnable treeDiffRunnable = new OptimizedTreeDifferenceRunnable(oldNodeList,
						newNodeList, subList, treeDiffCounter, stringSimCache, onlyOneClassPair, orderedList1,
						orderedList2, resultMap, similarityCache, similarityEntries, parents1, parents2, leavesMap1,
						leavesMap2, labelConfiguration, lMatcher, directChildrenMap1, directChildrenMap2, getSrc(),
						getDst(), configuration.weightSimilarity, configuration.weightPosition);
				if (oldNodeList.size() * newNodeList.size() > 10000000) {
					...
					if (oldNodeList.size() * newNodeList.size() < 12000000) {
						...
						FutureTask<Set<MatchingCandidate>> task = new FutureTask<>(treeDiffRunnable);
						task.run();
						...
						subList.clear();
						Collections.sort(tmpCandidates, new PairComparator(orderedList1, orderedList2));
	...
	public void matchUnchangedCode(IdentityHashMap<ITree, Mapping> resultMap, Set<ITree> unmatchedNodes1,
			Set<ITree> unmatchedNodes2, ConcurrentHashMap<String, Float> stringSimCache, NGramCalculator stringSim,
			HashSet<ITree> skipList, Set<ITree> leaves1, Set<ITree> leaves2, LMatcher lMatcher,
			InnerNodeMatcher innerMatcher) {
		Set<ITree> tmp1O = new HashSet<>(leaves1);
		Set<ITree> tmp2O = new HashSet<>(leaves2);
		Set<ITree> tmp1L = new HashSet<>(leaves1);
		Set<ITree> tmp2L = new HashSet<>(leaves2);
		List<ITree> firstNodeList = getNodeStream(getSrc());
		List<ITree> secondNodeList = getNodeStream(getDst());

		for (int i = 0; i < Math.min(firstNodeList.size(), secondNodeList.size()); i++) {
			ITree firstNode = firstNodeList.get(i);
			ITree secondNode = secondNodeList.get(i);
			if (firstNode.getType() != secondNode.getType()) {
				break;
			}
			if (tmp1O.contains(firstNode) && tmp2O.contains(secondNode)) {
				float sim = 0.0f;
				sim = computeNodesimilarity(stringSimCache, stringSim, firstNode, secondNode, lMatcher);

				if (sim == 1.0f && lMatcher.match(firstNode, secondNode, sim)) {

					final MatchingCandidate candidate = new MatchingCandidate(firstNode, secondNode, sim);
					if (!resultMap.containsKey(candidate.first)) {
						resultMap.put(candidate.first, candidate.dropValue());
						skipList.add(firstNode);
						skipList.add(secondNode);
						unmatchedNodes1.remove(firstNode);
						unmatchedNodes2.remove(secondNode);
					}
				} else {
					break;
				}
			} else if (!tmp1L.contains(firstNode) && !tmp2L.contains(secondNode)) {
				final MatchingCandidate candidate = new MatchingCandidate(firstNode, secondNode, 1.0f);
				if (!resultMap.containsKey(candidate.first)) {
		...
		for (int i = 1; i < Math.min(firstNodeList.size(), secondNodeList.size()); i++) {
			ITree firstNode = firstNodeList.get(firstNodeList.size() - i);
			ITree secondNode = secondNodeList.get(secondNodeList.size() - i);
			if (firstNode.getType() != secondNode.getType()) {
				break;
			}
			if (tmp1O.contains(firstNode) && tmp2O.contains(secondNode)) {
	...
	public void newUnchangedMatching(LabelConfiguration labelConfiguration, IdentityHashMap<ITree, Mapping> resultMap,
			Set<ITree> unmatchedNodes1, Set<ITree> unmatchedNodes2, NGramCalculator stringSim,
			HashSet<ITree> skipList) {
		IdentityHashMap<ITree, Integer> quickFind = new IdentityHashMap<>();
		IdentityHashMap<ITree, String> stringMap = new IdentityHashMap<>();
		getHash(getSrc(), quickFind, stringMap);
		getHash(getDst(), quickFind, stringMap);
		HashMap<String, LinkedList<ITree>> nodeMapOld = new HashMap<>();
		List<ITree> streamOld = getNodeStream(getSrc());
		List<ITree> streamNew = getNodeStream(getDst());
		...
		for (Mapping pair : pairs) {
			List<ITree> stream1 = getNodeStream(pair.first);
			List<ITree> stream2 = getNodeStream(pair.second);
			...
			for (int i = 0; i < stream1.size(); i++) {
				...
				resultMap.put(oldNode, new Mapping(oldNode, newNode));
