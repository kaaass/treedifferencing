...
public class NGramCalculator {
	...
	public final int subMapSize;

	public NGramCalculator(final int n) {
		this.n = n;
		cache = new ConcurrentHashMap<>(10000);
		this.subMapSize = 3500200;
	}

	public NGramCalculator(final int n, int cacheSize, int subMapSize) {
		this.n = n;
		cache = new ConcurrentHashMap<>(cacheSize);
		this.subMapSize = subMapSize;
	}

	private HashSet<String> calculateNGrams(final String s, final int n) {
	...
	public float similarity(final String first, final String second) {
		if (first.equals(second)) {
			...
			final int m = Math.min(n, Math.min(first.length(), second.length()));
