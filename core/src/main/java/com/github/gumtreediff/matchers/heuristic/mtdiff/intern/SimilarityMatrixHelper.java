...
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.MtDiffOptimizedMatcher;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.hungarian.DoubleMatrix;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.hungarian.Hungarian;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.similarity.InnerNodeSimilarityCalculator;
import extern.com.github.gumtreediff.matchers.heuristic.mtdiff.similarity.NGramCalculator;
...
class SimilarityMatrixHelper {
      ...
      for (int i = 0; i < oldNodes.size(); i++) {
        for (int j = 0; j < newNodes.size(); j++) {
          if (root1.getId() > oldNodes.get(i).getId()) {
            ...
            int val = Math.abs((oldNodes.get(i).getId() - root1.getId())
                - (newNodes.get(j).getId() - root2.getId()));
            posDiff[i][j] = val;
            if (val > maxDiff) {
              maxDiff = val;
            }
   ...
   * todo.
   *
   *<p>Creates a {@see diff.tree.matching.NodeAggregation} that consists of the specified node, it's
   ...
   * todo.
   *
   *<p>A similarity calculation based on a simplified ChangeDistiller matching.
   */
