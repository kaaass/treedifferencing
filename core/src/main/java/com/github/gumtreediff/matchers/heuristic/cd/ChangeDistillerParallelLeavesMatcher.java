...
/**
 * 
 * Parallel variant of the leaves matcher.
 *
 */
public class ChangeDistillerParallelLeavesMatcher extends Matcher {
	...
	private class ChangeDistillerLeavesMatcherCallable implements Callable<ChangeDistillerCallableResult> {
		...
		public ChangeDistillerLeavesMatcherCallable(List<ITree> srcLeaves, List<ITree> dstLeaves, int cores,
				int start) {
	...
	public void match() {
		...
		for (int i = 0; i < cores; i++) {
			futures[i] = service.submit(new ChangeDistillerLeavesMatcherCallable(srcLeaves, dstLeaves, cores, i));
		}
		for (int i = 0; i < cores; i++) {
			try {
